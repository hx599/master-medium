...
def _selective_scan_fallback(xs, dts, As, Bs, Cs, Ds, delta_bias=None, delta_softplus=True):
    """Pure PyTorch fallback for environments without compiled selective-scan ops."""
    if delta_bias is not None:
        dts = dts + delta_bias.view(1, -1, 1).to(dts.dtype)
    if delta_softplus:
        dts = F.softplus(dts)

    batch_size, dim, length = xs.shape
    _, num_groups, d_state, _ = Bs.shape
    if dim % num_groups != 0:
        raise ValueError(f'Expected dim {dim} to be divisible by num_groups {num_groups}.')

    group_width = dim // num_groups
    As = As.to(xs.dtype).unsqueeze(0)
    Ds = Ds.to(xs.dtype).view(1, dim)
    state = xs.new_zeros(batch_size, dim, d_state)
    outputs = []

    for t in range(length):
        u_t = xs[:, :, t]
        dt_t = dts[:, :, t]
        b_t = Bs[:, :, :, t].unsqueeze(2).repeat(1, 1, group_width, 1).reshape(batch_size, dim, d_state)
        c_t = Cs[:, :, :, t].unsqueeze(2).repeat(1, 1, group_width, 1).reshape(batch_size, dim, d_state)

        delta_a = torch.exp(dt_t.unsqueeze(-1) * As)
        delta_b_u = dt_t.unsqueeze(-1) * b_t.to(xs.dtype) * u_t.unsqueeze(-1)
        state = delta_a * state + delta_b_u
        y_t = (state * c_t.to(xs.dtype)).sum(dim=-1) + Ds * u_t
        outputs.append(y_t)
...
if 'selective_scan_fn_v1' not in globals():
    selective_scan_fn_v1 = _selective_scan_fallback
if 'selective_scan_ref_v1' not in globals():
    selective_scan_ref_v1 = _selective_scan_fallback
