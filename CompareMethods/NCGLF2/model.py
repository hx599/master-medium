...
class RelativeCoordPredictor(nn.Module):
    ...
    def forward(self, x):
        ...
        basic_index = torch.from_numpy(np.array([i for i in range(N)])).to(device=device, dtype=torch.long)

        basic_label = torch.from_numpy(self.build_basic_label(size)).float()
        # Build Label
        label = basic_label.to(device)
...
class CNN_Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x_out = torch.empty(x.shape[0], dim, p, p, device=x.device)
        for i in range(dim):
...
class NCGLF(nn.Module):
    ...
    def forward(self, x):
        ...
        if c != self.c1 + self.c2:
            raise ValueError(
                f'Expected {self.c1 + self.c2} input channels, but got {c}. '
                f'NCGLF2 now splits the fused input as primary={self.c1}, secondary={self.c2}.'
            )
        hsi = x[:, :self.c1, :, :]
        lidar = x[:, self.c1:self.c1 + self.c2, :, :]
